...
- 1行が練習の1単位。複数の文を1行に書けば、まとめて1セクションとして練習
- メモ・タグの付与、キーワード検索
- JSONでの書き出し / 読み込み
...
- **1行がそのまま練習の1単位**です。アプリは勝手に文を分割しないので、複数の文を1行に書けば1セクションとしてまとめて練習できます
- 1行に入れた複数の文をあとから分けたい場合は「1行の中も文ごとに分けて登録する」をオンにすると、英日それぞれ文ごとに分割して対応させます(数が合わない場合はエラーになります)
